...
const nextConfig = {
  ...
  // fixing both the missing-chunk 500 and the "two React copies" hook crash
  // that appear when running under pnpm's virtual symlink store.
  transpilePackages: ["motion", "framer-motion"],
  // Allow Replit's iframe proxy to fetch /_next/* assets without being
  // blocked by Next.js 15.5's cross-origin dev-server guard.
  allowedDevOrigins: [
    "*.replit.dev",
    "*.spock.replit.dev",
    "*.kirk.replit.dev",
