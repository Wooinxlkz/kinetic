...
type Props = {
  ...
  filename?: string;
  className?: string;
  /** Component slug for per-component daily quota tracking (e.g. "orbiting-circles"). */
  slug?: string;
};
    ...
    >
      {filename ? (
        <div className="flex items-center justify-between gap-3 border-b border-border bg-background/60 px-4 py-2.5">
          ...
          <CopyButton text={code} eventLabel={filename ?? lang} slug={slug} />
        </div>
      ) : (
