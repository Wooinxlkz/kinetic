...
// so kinetic's own Next.js route handlers must avoid that prefix.
...
/** Hidden dev sign-in — PIN + name, verified server-side against secrets. */
export function devSignIn(pin: string, name: string): Promise<AuthUser> {
  return request("/auth/dev-login", { body: JSON.stringify({ pin, name }) });
}
...
/** Changes the signed-in user's password. Throws AuthApiError on failure. */
export async function changePassword(
  ...
  if (!res.ok) {
    throw new AuthApiError(
      (data && typeof data.error === "string" && data.error) || "Failed to change password",
    );
  }
}

/** Permanently deletes the signed-in user's account after email confirmation. */
export async function deleteAccount(email: string): Promise<void> {
  ...
  if (!res.ok) {
    throw new AuthApiError(
      (data && typeof data.error === "string" && data.error) || "Failed to delete account",
    );
  }
}
