...
function SignInForm({
  ...
  return (
    ...
    >
      ...
      {/* Deliberately near-invisible — a hidden entry point, not a feature to advertise. */}
      <div className="flex justify-center pt-1">
        <button
          type="button"
          onClick={onDevClick}
          aria-label="Dev sign-in"
          className="text-[10px] uppercase tracking-wider text-white/10 transition-colors duration-200 hover:text-white/40"
        >
          •
        </button>
      </div>
...
export function AuthModal() {
  ...
  const [view, setView] = useState<"sign-in" | "sign-up" | "dev">(defaultTab ?? "sign-in");
  ...
  const heading = view === "sign-in" ? "Welcome back" : view === "sign-up" ? "Create account" : "Dev access";
  const subline =
    view === "sign-in"
                  ...
                  >
                    {view === "sign-in" && (
                      <SignInForm
                        ...
                        onDevClick={() => setView("dev")}
                        initialEmail={prefillEmail ?? ""}
                      />
                    )}
                    {view === "sign-up" && <SignUpForm onSwitch={() => setView("sign-in")} />}
                    {view === "dev" && <DevLoginForm onClose={() => setView("sign-in")} />}
                  </motion.div>
                </AnimatePresence>
              </div>
            </motion.div>
