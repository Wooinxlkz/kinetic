...
export interface PreviewRailProps {
  ...
  /**
   * Fix: previously this rail was ALWAYS fully clickable/hoverable, even
   * when rendered at near-zero opacity as a passive "ghost" preview.
   * That's what was intercepting hover/clicks meant for content underneath
   * it. Pass `interactive={false}` while it's just a faint visual, and
   * `interactive={true}` only once the user has actually engaged the
   * trigger. Defaults to true so existing usages aren't silently broken.
   */
  interactive?: boolean;
}
    ...
    <motion.div
      ...
      className={cn(
        ...
        isHorizontal
          ...
          : "min-h-80",
        // Fix: this is what actually stops the "ghost" state from being
        // clickable/hoverable. Without this, the rail was always fully
        // live underneath the sidebar, even at 8% opacity.
