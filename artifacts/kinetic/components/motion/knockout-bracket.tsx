"use client";
// kinetic/components/blocks/knockout-bracket

import { AnimatePresence, motion, useReducedMotion } from "motion/react";
import { ChevronLeft, ChevronRight, Shield, Trash2, X } from "lucide-react";
...
export interface KnockoutBracketProps {
  ...
  initialRound?: number;
  /** Lets viewers click any card to edit teams, scores, and status in place. Defaults to true. */
  editable?: boolean;
  className?: string;
}
...
// ── flag picker data ─────────────────────────────────────────────────────
// Every code below was checked against flagcdn's own registry
// (flagcdn.com/en/codes.json) — each one resolves to a real flag image, none
// are broken or silently hidden. Palestine ("ps") is included; Israel is
...
const COUNTRIES: [name: string, code: string][] = [
  ...
  ["Brazil", "br"], ["Bulgaria", "bg"], ["Cabo Verde", "cv"], ["Cameroon", "cm"],
  ...
  ["Greece", "gr"], ["Guinea", "gn"], ["Honduras", "hn"], ["Hungary", "hu"],
  ["Iceland", "is"], ["India", "in"], ["Indonesia", "id"], ["Iran", "ir"],
  ["Iraq", "iq"], ["Ireland", "ie"], ["Italy", "it"], ["Ivory Coast", "ci"],
  ["Jamaica", "jm"], ["Japan", "jp"], ["Jordan", "jo"], ["Kenya", "ke"],
  ["Kosovo", "xk"], ["Kuwait", "kw"], ["Lebanon", "lb"], ["Libya", "ly"],
  ["Luxembourg", "lu"], ["Malaysia", "my"], ["Mali", "ml"], ["Mexico", "mx"],
...
function TeamFlag({ code, size = 20 }: { code?: string | null; size?: number }) {
  ...
  return (
    ...
    <img
      ...
      className="shrink-0 rounded-[4px] border border-border/40 object-cover"
      style={{ height: size, width: w }}
    />
  );
}
...
function matchLabel(roundName: string, m: Match, editable: boolean) {
  const sides =
    m.status === "finished"
  ...
  const suffix = editable ? ", click to edit" : "";
  return `${roundName}: ${sides}${when}${outcome}${suffix}`;
}
...
function MatchCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={matchLabel("", match, true)}
      style={{ width: CARD_W, height: CARD_H }}
      className="rounded-2xl border border-border bg-card p-4 text-left transition-colors hover:bg-card/70 hover:border-border/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    >
      {body}
    </button>
  );
...
// ── searchable flag combobox used inside the edit sheet ────────────────────
function FlagPicker({
  code,
  onPick,
}: {
  ...
  const selectedName = COUNTRIES.find(([, c]) => c === code)?.[0];
  ...
  return (
    <div ref={boxRef} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-2 rounded-lg border border-border bg-background px-2.5 py-2 text-left text-sm text-foreground"
      >
        <TeamFlag code={code} size={16} />
        <span className="flex-1 truncate">{selectedName ?? "Choose flag"}</span>
      </button>
      {open && (
        <div className="absolute z-30 mt-1 w-full">
          <input
            ...
            className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          />
          <div className="mt-1 max-h-44 overflow-y-auto rounded-lg border border-border bg-background">
            {filtered.map(([name, c]) => (
              <div
                key={c + name}
                onClick={() => {
                ...
                className="flex cursor-pointer items-center gap-2 px-2.5 py-1.5 text-sm text-foreground hover:bg-foreground/5"
              >
                <TeamFlag code={c} size={14} />
                <span>{name}</span>
              </div>
            ))}
            {filtered.length === 0 && (
...
function SideEditor({
  ...
  label: string;
  side: MatchSide;
  onChange: (patch: Partial<MatchSide>) => void;
}) {
  return (
    <div className="space-y-2.5 rounded-xl border border-border/70 p-3">
      ...
      <input
        placeholder="Team name (any name — not limited to a fixed list)"
        value={side.team?.name ?? ""}
        onChange={(e) =>
          onChange({ team: { code: side.team?.code ?? "", name: e.target.value } })
        }
        className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
      />
      <FlagPicker
        code={side.team?.code}
        onPick={(code) =>
          onChange({ team: { name: side.team?.name ?? "", code } })
        }
      />
      <div className="grid grid-cols-2 gap-2.5">
        <input
          ...
          value={side.score ?? ""}
          onChange={(e) =>
            onChange({ score: e.target.value === "" ? null : Number(e.target.value) })
          }
          className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm tabular-nums text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
        />
        <input
          type="number"
          min={0}
          placeholder="Penalties"
          value={side.penalties ?? ""}
          onChange={(e) =>
            onChange({
              penalties: e.target.value === "" ? null : Number(e.target.value),
            })
          }
          className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm tabular-nums text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
        />
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className="max-h-[88vh] w-full max-w-md overflow-y-auto rounded-2xl border border-border bg-card p-5"
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-bold text-foreground">Edit match</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <label className="space-y-1 text-xs font-medium text-muted-foreground">
              ...
              <select
                value={match.status}
                onChange={(e) =>
                  onChange({ status: e.target.value as Match["status"] })
                }
                className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
            ...
            <label className="space-y-1 text-xs font-medium text-muted-foreground">
              ...
              <select
                ...
                onChange={(e) =>
                  onChange({
                    winner: (e.target.value || undefined) as Match["winner"],
                  })
                }
                className="w-full rounded-lg border border-border bg-background px-2.5 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              >
          ...
          <SideEditor
            label="Home"
            side={match.home}
            onChange={(patch) => onChange({ home: { ...match.home, ...patch } })}
          />
          <SideEditor
            label="Away"
            side={match.away}
            onChange={(patch) => onChange({ away: { ...match.away, ...patch } })}
          />

          <button
            type="button"
            onClick={onClear}
            className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:border-destructive/40 hover:text-destructive"
          >
...
const EMPTY_MATCH_BASE: Omit<Match, "id"> = {
  date: "TBD",
  status: "upcoming",
  home: { team: null, score: null, penalties: null },
  away: { team: null, score: null, penalties: null },
};
...
export function KnockoutBracket({
  ...
  // Uncontrolled: seeded from `rounds` on mount, then edited locally — so the
  // demo stays a one-liner (`<KnockoutBracket rounds={ROUNDS} />`) with no
  // extra state wiring in the parent.
  const [data, setData] = useState<Round[]>(rounds);
  const [editing, setEditing] = useState<{ r: number; k: number } | null>(null);

  const visibleCols = Math.min(VISIBLE_COLS, data.length);
  ...
  const maxPage = Math.max(0, data.length - Math.min(2, data.length));
  ...
  const { cy, containerHeight, connectors } = useMemo(() => {
    ...
    for (let r = page + 1; r < data.length; r++) {
      centers[r] = data[r].matches.map(
    ...
    for (let r = page - 1; r >= 0; r--) {
      ...
      centers[r] = data[r].matches.map((_, i) => {
  ...
  function patchMatch(r: number, k: number, patch: Partial<Match>) {
    setData((prev) => {
      const next = prev.map((round) => ({
        ...round,
        matches: round.matches.map((m) => ({ ...m, home: { ...m.home }, away: { ...m.away } })),
      }));
      next[r].matches[k] = { ...next[r].matches[k], ...patch };
      return next;
    });
  }

  function clearMatch(r: number, k: number) {
    setData((prev) => {
      const next = prev.map((round) => ({
        ...round,
        matches: round.matches.map((m) => ({ ...m })),
      }));
      const id = next[r].matches[k].id;
      next[r].matches[k] = { ...EMPTY_MATCH_BASE, id };
      return next;
    });
  }

  const editingMatch = editing ? data[editing.r].matches[editing.k] : null;
    ...
    >
      ...
      <AnimatePresence>
        {editingMatch && editing && (
          <EditModal
            match={editingMatch}
            onChange={(patch) => patchMatch(editing.r, editing.k, patch)}
            onClose={() => setEditing(null)}
            onClear={() => {
              clearMatch(editing.r, editing.k);
              setEditing(null);
            }}
          />
        )}
