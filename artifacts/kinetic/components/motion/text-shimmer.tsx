...
export function TextShimmer({ children, as: Comp = "span", duration = 2.5, className }: TextShimmerProps) {
  return (
    <>
      <style>
        {`@keyframes kinetic-text-shimmer{from{background-position:200% 0}to{background-position:-200% 0}}`}
      </style>
      <Comp
        style={{ animation: `kinetic-text-shimmer ${duration}s linear infinite` }}
        className={cn(
