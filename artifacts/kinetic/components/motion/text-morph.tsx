...
export interface TextMorphCycleItem {
  /** The text to display. */
  label: string;
  /** How long this label stays on screen, in milliseconds. */
  duration: number;
}

export interface TextMorphCycleProps {
  items: TextMorphCycleItem[];
  as?: ElementType;
  className?: string;
  style?: CSSProperties;
}
...
/**
 * Cycles through a sequence of labels, each held for its own duration,
 * morphing between them with `TextMorph`. Useful for a nav item that wants
 * to rotate between related labels (e.g. "Membership" -> "Pro" -> "Sponsor").
 */
export function TextMorphCycle({
  items,
  as = "span",
  className,
  style,
  ...
  return (
    <TextMorph as={as} className={className} style={style}>
      {current.label}
    </TextMorph>
  );
}
