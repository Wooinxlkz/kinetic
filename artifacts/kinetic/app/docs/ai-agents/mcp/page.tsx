...
claude mcp add --transport http kinetic ${MCP_URL}

# Codex
codex mcp add kinetic --url ${MCP_URL}

# Amp
amp mcp add kinetic ${MCP_URL}

# Cursor / any JSON config
{
  "mcpServers": {
    "kinetic": { "type": "http", "url": "${MCP_URL}" }
  }
}`;
...
await mcp.callTool("kinetic", "search_components", { query: "toast notification" });
...
// Example
await mcp.callTool("kinetic", "get_component", { slug: "animated-toast-stack" });
...
// Response
{
  ...
  "category": "motion",
  "page_url": "https://kinetic.dev/components/motion/animated-toast-stack",
  "detail_url": "https://kinetic.dev/r/animated-toast-stack",
  "raw_url":    "https://kinetic.dev/r/animated-toast-stack/raw",
...
await mcp.callTool("kinetic", "get_install_command", {
  slug: "tilt-card",
  pm: "pnpm",
});

// Response
"pnpm dlx shadcn@latest add https://kinetic.dev/r/tilt-card.json"`;

const AGENT_FLOW = `// Typical agent workflow using MCP tools

// 1. User: "add an animated notification component"
const results = await mcp.callTool("kinetic", "search_components", {
  query: "notification toast"
});

// 2. Agent picks the best match
const component = await mcp.callTool("kinetic", "get_component", {
  slug: results[0].slug
});
