...
const MCP_URL = "https://mcp.kinetic.dev/mcp";

const MCP_CLI_SNIPPET = `# Claude Code
claude mcp add --transport http kinetic https://mcp.kinetic.dev/mcp

# Codex
codex mcp add kinetic --url https://mcp.kinetic.dev/mcp

# Amp
amp mcp add kinetic https://mcp.kinetic.dev/mcp`;

const MCP_MANUAL_SNIPPET = `{
  "mcpServers": {
    "kinetic": {
      "type": "http",
      "url": "https://mcp.kinetic.dev/mcp"
    }
  }
}`;

const FETCH_SNIPPET = `// 1. Discover what exists
const idx = await fetch('https://kinetic.dev/r').then((r) => r.json());

// 2. Fetch a component
const entry = await fetch(\`https://kinetic.dev/r/\${slug}\`).then((r) => r.json());
...
npx shadcn@latest add https://kinetic.dev/r/animated-toast-stack.json`;
...
const ENTRY_SHAPE = `{
  ...
  "category": "motion",
  "page_url": "https://kinetic.dev/components/motion/swap",
  "detail_url": "https://kinetic.dev/r/swap",
  "raw_url": "https://kinetic.dev/r/swap/raw",
  ...
  return (
    <>
      ...
      <p className="mt-3 text-muted-foreground">
        Kinetic exposes a static, agent-friendly surface. Connect the MCP server
      ...
      <p className="mt-2 text-muted-foreground">
        The fastest path: connect the Kinetic MCP server and your agent can list,
      ...
      <p className="mt-2 text-muted-foreground">
        ...
        the target app&apos;s theme without Kinetic-specific color variables.
      </p>
      <div className="mt-4">
