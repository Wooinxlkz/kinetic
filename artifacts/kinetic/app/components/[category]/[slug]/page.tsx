...
import { PreviewContainer } from "@/components/app/docs/preview-container";
...
export async function generateMetadata({
  ...
  return {
    ...
    other: {
      "kinetic:category": cat.slug,
      "kinetic:component": comp.slug,
      "kinetic:registry-item": registryItem,
      "kinetic:directory-item": directoryItem,
      ...(installSlugs.length > 0
        ? {
            "kinetic:variant-registry-items": installSlugs.map(
    ...
    comp.examples?.map((example) => ({
      ...
      children: [
        ...
        ...(getComponentProps(example.file).length
          ? [{ id: `${example.slug}-api-reference`, label: "API Reference" }]
          : []),
      ],
    })) ?? [];
      ...
      <div className="min-w-0">
        ...
        <div id="overview" className="scroll-mt-24">
          ...
          >
            ...
            {comp.group && (() => {
              const groupTag = comp.group.toLowerCase();
              const isFilterable = CATEGORY_TAG_SETS[cat.slug]?.includes(groupTag);
              return (
                <>
                  <ChevronRight className="h-3.5 w-3.5 text-muted-foreground" />
                  {isFilterable ? (
                    <Link
                      href={`/components/${cat.slug}?tag=${groupTag}`}
                      className="text-muted-foreground transition-colors hover:text-foreground"
                    >
                      {comp.group}
                    </Link>
                  ) : (
                    <span className="text-muted-foreground">{comp.group}</span>
                  )}
                </>
              );
            })()}
      ...
      <div id={`${example.slug}-preview`} className="scroll-mt-24">
        <Tabs defaultValue="preview" variant="pill">
          ...
          <TabsContent value="preview" className="mt-4">
            <PreviewContainer minHeight="260px">
              {Preview ? <Preview /> : null}
            </PreviewContainer>
          </TabsContent>
          <TabsContent value="usage" className="mt-4">
            <CodeBlock code={usage} filename={example.previewFile} slug={pageSlug} />
          </TabsContent>
  ...
  return (
    <section id="preview" className="mt-8 scroll-mt-24">
      <Tabs defaultValue="preview" variant="pill">
        ...
        <TabsContent value="preview" className="mt-4">
          <PreviewContainer minHeight="320px">
            {Preview ? <Preview /> : null}
          </PreviewContainer>
        </TabsContent>
        <TabsContent value="usage" className="mt-4">
          <CodeBlock code={usage} filename={previewFile} slug={slug} />
        </TabsContent>
        <TabsContent value="source" className="mt-4">
          <CodeBlock
